...
// 裁剪相关的函数已移除，现在直接在 chooseAvatar 中处理上传
...
const loadUserProfile = async () => {
  try {
    ...
    Object.assign(userProfile, {
      ...
      realName: response.username || '', // 后端返回的真实姓名字段
      nickname: response.nickname,
      gender: response.gender,
      role: response.role,
      grade: response.grade || 0,
      college: response.college || '计算机学院',
      phone: response.phone,
      email: response.email,
      bio: response.introduction,
      banEndTime: response.banned_at || -1,
      avatar: avatarUrl
    })
      ...
      <wd-cell-group border custom-class="mb-3 rounded-none">
        <wd-cell-group title="个人信息" custom-class="bg-white">
          ...
          <wd-cell title="年级" center clickable @click="chooseGrade">
            <template #icon>
              <WdIcon name="school" size="18" color="#3b82f6" class="mr-2" />
            </template>
            <template #right-icon>
              <view class="flex items-center">
          ...
          <wd-cell title="学院" center>
            <template #icon>
              <WdIcon name="university" size="18" color="#3b82f6" class="mr-2" />
            </template>
            <wd-input 
